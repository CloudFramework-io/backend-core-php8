...
class API extends RESTful
{
    ...
    function main() {
        ...
        $path = $this->core->config->get('bucketUploadPathTest');
        if(!is_dir($path)) return $this->setErrorFromCodelib('system-error','$this->core->config->get(\'bucketUploadPathTest\') is not accesible: '.$this->core->config->get('bucketUploadPathTest'));

        $this->addReturnData(['path'=>$this->core->config->get('bucketUploadPathTest')]);

        if(!$this->checkMandatoryFormParams(['file1','file2','output'])) return;
        ...
        if(!$this->buckets->isFile('/'.$this->formParams['file1'])) return($this->setErrorFromCodelib('params-error','file1 does not exist: '.$this->formParams['file1']));
        if(!$this->buckets->isFile('/'.$this->formParams['file2'])) return($this->setErrorFromCodelib('params-error','file1 does not exist: '.$this->formParams['file2']));
    ...
    private function withClsTbsZip(){

        /** @var clsTbsZip $zip */
        $zip = new clsTbsZip(); // create a new instance of the TbsZip class
        $zip->CreateNew(); // start a new empty archive for adding files

        $zip->FileAdd($this->formParams['file1'], $this->core->config->get('bucketUploadPathTest').'/'.$this->formParams['file1'], TBSZIP_FILE);
        $zip->FileAdd($this->formParams['file2'], $this->core->config->get('bucketUploadPathTest').'/'.$this->formParams['file2'], TBSZIP_FILE);
        $zip->Flush(TBSZIP_FILE, $this->core->config->get('bucketUploadPathTest').'/'.$this->formParams['output']); // apply modifications as a new local file
        $zip->Close();
    }

    private function withZipArchive() {

        $this->addReturnData(['ZipArchive'=>class_exists('ZipArchive')]);


        $zip = new ZipArchive();
        $filename = $this->core->config->get('bucketUploadPathTest').'/'.$this->formParams['output'];

        if ($zip->open($filename, ZipArchive::OVERWRITE)!==TRUE) {
            exit("cannot open <$filename>\n");
        }
        if(!$zip->addFile($this->core->config->get('bucketUploadPathTest').'/'.$this->formParams['file1'],$this->formParams['file1'],0,filesize($this->core->config->get('bucketUploadPathTest').'/'.$this->formParams['file1']))) {
            //return($this->setErrorFromCodelib('system-error','error adding '.$this->formParams['file1'].' Error: '.$zip->getStatusString()));
        }
        $zip->addFile($this->core->config->get('bucketUploadPathTest').'/'.$this->formParams['file2'],$this->formParams['file2'],0,filesize($this->core->config->get('bucketUploadPathTest').'/'.$this->formParams['file1']));
        $this->addReturnData(['numfiles'=>$zip->numFiles,'status'=>$zip->status]);
        $this->addReturnData(['msg'=>$zip->getStatusString(),'close'=>$zip->close()]);
    }
}
